...
namespace WAIUA.Commands
{
    ...
    public class Main
    {
        ...
        private static string DoCachedRequest(Method method, string url, bool addRiotAuth, bool bypassCache = false)
        {
            ...
            if (attemptCache)
                if (Session.UrlToBody.TryGetValue(url, out var res))
                    return res;
            ...
            if (attemptCache) Session.UrlToBody.TryAdd(url, cont);
            return cont;
        ...
        private static void LiveMatchSetup()
        {
            ...
            if (Session.GamePodsDictionary.TryGetValue(matchinfo.GamePodID, out string ServerName))
            {
                Server = ServerName;
            }
        ...
        {
            if (IsIncognito[playerno] && PartyList[playerno] != PPartyID)
            {
                PlayerList[playerno] = "----";
        ...
        private static void GetPlayerHistory(string puuid, sbyte playerno)
        {
            ...
            if (!string.IsNullOrEmpty(puuid))
            {
                ...
                var content = DoCachedRequest(Method.GET, $"https://pd.{Region}.a.pvp.net/mmr/v1/players/{puuid}", true,
                    true);
