	...
	public class Main
	{
		...
		private static void GetAgentInfo(string agent, sbyte playerno)
		{
			try
			{
				if (!String.IsNullOrEmpty(agent))
				{
					...
					{
						if (agentEntry.uuid != agent) continue;
						AgentPList[playerno] = agentEntry.killfeedPortrait;
		...
		private static void GetCardInfo(string card, sbyte playerno)
		{
			try
			{
				...
				foreach (var skin in content.data)
				{
					if (skin.uuid != card) continue;
					CardList[playerno] = skin.smallArt;
					break;
				}
			}
			finally
		...
		private static void GetSkinInfo(sbyte playerno)
		{
			try
			{
				if (!String.IsNullOrEmpty(PUUIDList[playerno]))
				{
					...
					{
						string vandalcontent = DoCachedRequest(Method.GET,
							$"https://valorant-api.com/v1/weapons/skinchromas/{vandalchroma}?language={Language}",
							false);
					...
					{
						string phantomcontent = DoCachedRequest(Method.GET,
							$"https://valorant-api.com/v1/weapons/skinchromas/{phantomchroma}?language={Language}",
							false);
		...
		private static void GetRankStuff(sbyte playerno, string rank, string prank, string pprank, string ppprank)
		{
			...
			foreach (var tiers in content.data[3].tiers)
			{
				if (tiers.tier == rank)
				{
					RankList[playerno] = tiers.largeIcon;
					RankNameList[playerno] = tiers.tierName;
				}
				else if (tiers.tier == prank)
				{
					PRankList[playerno] = tiers.smallIcon;
					PRankNameList[playerno] = tiers.tierName;
				}
				else if (tiers.tier == pprank)
				{
					PPRankList[playerno] = tiers.smallIcon;
					PPRankNameList[playerno] = tiers.tierName;
				}
				else if (tiers.tier == ppprank)
				{
					PPPRankList[playerno] = tiers.smallIcon;
